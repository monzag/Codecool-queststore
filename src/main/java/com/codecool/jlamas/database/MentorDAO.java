...
public class MentorDAO {
    ...
    public ArrayList<Mentor> requestAll() {
        String query = String.format("%s %s %s %s %s %s %s"
            , "SELECT user.login, user.email, user.name, user.surname, login.password, mentor.groupTag"
            , "FROM user"
            ,     "INNER JOIN login"
        ...
        try (Connection c = ConnectDB.connect();
            ...
            while (rs.next()) {
                ...
                mentor.setGroup(new Group(rs.getString("groupTag")));
    ...
    public boolean update(Mentor mentor) {
        ...
        try (Connection c = ConnectDB.connect();
            ...
            query += String.format("UPDATE `mentor` SET login = '%s', groupTag = '%s' WHERE login = '%s'; ",
                    mentor.getLogin().getValue(),
                    mentor.getGroup().getName(),
                    mentor.getLogin().getValue());
